    ...
    <el-row :gutter="24" >
      ...
      <el-col :span="6" v-for="(item,index) in listdata" :key="index"  style="padding-bottom: 18px;">
        <el-card :body-style="{ padding: '8px' }" shadow="hover"
                 @click.native=" item.status.status === 'running' && open(item.imageId,item.imageVulName,item.imageDesc,item.status.status,item.status.containerId,item)" >
          <div class="clearfix" >
            <div style="display: inline-block;height: 20px;line-height: 20px;min-height: 20px;max-height: 20px;">
              ...
              <div style="display: inline-block;margin: 0;" v-if="item.status.status === 'running' && item.status.start_date !== null && item.status.start_date !=='' && item.status.end_date !== null && item.status.end_date !== '' && item.status.end_date !== 0">
                ...
                <count-down style="display: inline-block;height: 20px;line-height: 20px;size: 20px;margin-block-start: 0em;margin-block-end: 0em;" v-on:end_callback="stop(item.status.containerId, item)" :currentTime="item.status.now" :startTime=item.status.now :endTime=item.status.end_date :secondsTxt="''"></count-down>
              </div>
              <div style="display: inline-block;" v-else-if="item.status.status === 'running' && item.status.start_date !== null && item.status.start_date !=='' && item.status.endDate !== null && item.status.endDate !== '' && item.status.endDate === 0">
                <el-tooltip content="容器剩余时间，0 为用不过期" placement="top">
                  <i class="el-icon-time"></i>
                </el-tooltip>
          ...
          <div style="padding: 5px;" >
            ...
            <span>{{item.imageVulName}}</span>
            </div>
            <div class="bottom clearfix">
              <div class="time container-title">{{ item.imageDesc }}</div>
            </div>
            <el-row>
              ...
              <el-button type="primary" @click.stop="open(item.imageId,item.imageVulName,item.imageDesc,item.status.status,item.status.container_id,item)" :disabled="item.status.start_flag" size="mini" v-else="item.status.status === '' || item.status.status === 'stop'">启动</el-button>
              ...
              getTask(taskId).then(response=>{
                ...
                if (responseStatus === 1001){
                  ...
                  if (responseStatus === 200){
                    ...
                    this.vul_port = JSON.parse(responseData["data"]["port"])
