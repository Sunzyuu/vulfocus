  ...
  <div class="app-container">
          ...
          <el-tab-pane name="add" label="添加">
            <el-form label-width="80px"
              ...
              <el-form-item label="漏洞名称">
                <el-input v-model="vulInfo.vulName" size="medium"></el-input>
              </el-form-item>
    ...
    <el-dialog :visible.sync="editShow" title="修改">
      <el-form label-width="80px" v-loading="editLoding" element-loading-text="修改中">
        <el-form-item label="漏洞名称">
          <el-input v-model="editVulInfo.imageVulName" size="medium"></el-input>
        </el-form-item>
        <el-form-item label="镜像">
          <el-input v-model="editVulInfo.imageName" disabled></el-input>
        </el-form-item>
        <el-form-item label="Rank">
        ...
        <el-form-item label="描述">
          <el-input type="textarea" v-model="editVulInfo.imageDesc" size="medium"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="tableData" border stripe align = "center" style="width: 100%">
      <el-table-column type="index" width="50"> </el-table-column>
      <el-table-column prop="imageName" label="镜像名称" :show-overflow-tooltip=true ></el-table-column>
      <el-table-column prop="imageVulName" label="漏洞名称" :show-overflow-tooltip=true></el-table-column>
      <el-table-column prop="imagePort" label="端口" width="150"></el-table-column>
      <el-table-column prop="rank" label="分数" width="50"></el-table-column>
      <el-table-column prop="imageDesc" :show-overflow-tooltip=true label="描述"> </el-table-column>
      <el-table-column fixed="right" label="操作" width="280">
        <template slot-scope="{row}">
          ...
          <el-button style="display: inline-block;float: left;margin-left: 5px;"
                     v-else-if="row.ok === false && row.status.task_id === ''"
                     size="mini"
                     type="primary"
                     icon="el-icon-download"
                     @click="downloadImg(row)">下载</el-button>
          <el-button style="display: inline-block;float: left;margin-left: 5px;"
                     v-if="(row.ok === true) || (row.ok === false && row.status.task_id === '')" size="mini"
                     icon="el-icon-edit"
                     type="primary"
                     @click="openEdit(row)">修改</el-button>
          <el-button style="display: inline-block;float: left;margin-left: 5px;"
            v-if="(row.ok === true) || (row.ok === false && row.status.task_id === '')" size="mini" type="danger"
            icon="el-icon-delete"
            @click="handleDelete(row)">删除</el-button>
<!--          <el-tag style="display: inline-block;float: left;line-height: 28px;height: 28px; margin-left: 5px;"-->
<!--                  type="success" effect="dark" v-if="row.ok === true && row.is_share === true">-->
<!--            <div style="display: inline-block;float: left"><span>已分享</span></div>-->
<!--          </el-tag>-->
<!--          <el-button style="display: inline-block;float: left;margin-left: 5px;"-->
<!--                     v-if="(row.ok === true && row.is_share === false && row.status.progress_status !== 'share')"-->
<!--                     size="mini"-->
<!--                     type="primary"-->
<!--                     icon="el-icon-share"-->
<!--                     @click="shareImg(row)">分享</el-button>-->
<!--          <el-tag style="display: inline-block;float: left;line-height: 28px;height: 28px; margin-left: 5px;"-->
<!--                  @click="openProgress(row,2)" effect="dark" v-if="row.ok === true && row.status.progress_status === 'share'">-->
    ...
    methods:{
      ...
      initTableData(){
        ...
        ImgList(undefined, true, 1).then(response => {
          ...
          // let tmpTableData = response.data.data.records
      ...
      handleEditImage(){
        ...
        ImageEdit(this.editVulInfo.imageId,this.editVulInfo).then(response => {
      ...
      uploadImg() {
        ...
        formData.set("imageName", this.vulInfo.name)
        formData.set("imageVulName", this.vulInfo.vulName)
        formData.set("imageDesc", this.vulInfo.desc)
        this.loading = true
      ...
      handleDelete(row){
        this.$confirm('确认删除?', '提示', {
          ...
          ImageDelete(row.imageId).then(response => {
            let data = response.data
            if(data.status === 200){
              this.$message({
                title: '成功',
      ...
      handleSelect(item){
        ...
        this.vulInfo.vulName = item.value.replace("vulfocus/", "")
