...
export default {
    ...
    upload(){
      ...
      uploadImage(data).then(response => {
        ...
        if (rsp.data && rsp.status === 200){
          ...
          this.layout.imageName = process.env.VUE_APP_BASE_API + '/static/images/' + rsp.data
        }else{
          this.$message({
            message: rsp.msg,
            type: 'error'
  ...
  mounted() {
    if (this.$route.query.layoutData !== null && this.$route.query.layoutData !== undefined && this.$route.query.layoutData.layoutId){
      this.topoData = JSON.parse(this.$route.query.layoutData.rawContent)
      this.layout.id = this.$route.query.layoutData.layoutId
      this.layout.name = this.$route.query.layoutData.layoutName
      this.layout.desc = this.$route.query.layoutData.layoutDesc
      this.layout.imageName = this.$route.query.layoutData.imageName
    }else{
      this.layout = {
        id: '',
        name: '',
