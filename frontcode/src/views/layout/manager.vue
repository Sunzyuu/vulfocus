...
<div class="app-container">
  ...
  <el-table :data="tableData" border stripe style="width: 100%; margin-top: 20px">
    ...
    <el-table-column prop="layoutName" label="环境名称" width="180"></el-table-column>
    <el-table-column prop="layoutDesc" :show-overflow-tooltip=true label="环境描述" width="180"></el-table-column>
    <el-table-column label="图片" width="120px">
      <template slot-scope="{row}">
        <img @click="handleShowImage(row)" v-if="row.imageName" :src="row.imageName" style="width: 60px;height: 60px;display: block;"  alt=""/>
      </template>
    </el-table-column>
    <el-table-column
      label="日期"
      ...
      <template slot-scope="{row}">
        ...
        <span style="margin-left: 5px">{{ row.createDate }}</span>
      </template>
    </el-table-column>
    <el-table-column label="是否发布" width="85">
      <template slot-scope="{row}">
        <el-tag v-if="row.isRelease === true">已发布</el-tag>
        <el-tag v-else-if="row.isRelease === false">未发布</el-tag>
      </template>
    </el-table-column>
    <el-table-column fixed="right" label="操作">
      <template slot-scope="{row}">
        ...
        <el-button size="mini" type="primary" icon="el-icon-position" @click="handleRelease(row)" v-if="row.isRelease === false">发布</el-button>
        <el-button size="mini" type="primary" icon="el-icon-position" @click="handleRelease(row)" v-else-if="row.isRelease === true">取消发布</el-button>
  ...
  methods:{
    layoutListData(page){
      ...
      layoutList(this.search, page).then(response => {
        ...
        rsp.records.forEach((info,index) => {
          info.imageName = process.env.VUE_APP_BASE_API+ '/static/'+ info.imageName
          this.tableData.push(info)
        })
        this.page.total = rsp.total
      }).catch(err => {
        this.$message({
          type: 'error',
          message: '服务器内部错误!'
    ...
    handleDelete(row){
      this.$confirm('确认删除？删除会影响用户得分', '提示', {
        ...
        layoutDelete(row.layoutId).then(response => {
    ...
    handleShowImage(row){
      this.dialogImageUrl = row.imageName
      this.imageDialogVisible = true
    },
    handleShowYml(row){
      this.dialogYml = row.ymlContent
      this.ymlDialogVisible = true
    },
    handleEdit(row){
      this.$router.push({path:'/layout/index', query: {layoutId: row.layoutId, layoutData: row}})
    },
    handleRelease(row){
      layoutRelease(row.layoutId).then(response=>{
        let rsp = response.data
