  ...
  methods:{
    querySearchImageAsync(queryString, cb) {
      ...
      ImgList(queryString).then(response => {
        let results = response.data.data.records
        console.log(results)
        if(results !== null){
          results.forEach((item, index, arr) => {
            this.imageList.push({"value": item["imageName"], "data": item})
          });
        }
    ...
    handleImageSelect(item){
      ...
      this.image.id = imageData.imageId
      this.image.vulName = imageData.imageVulame
      this.image.name = imageData.imageName
      this.image.desc = imageData.imageDesc
      this.image.port = imageData.imagePort
    ...
    querySearchNetworkAsync(queryString, cb){
      ...
      NetWorkList(queryString, 1).then(response => {
        ...
        if(results !== null){
          results.forEach((item, index, arr) => {
            this.networkList.push({"value": item["netWorkName"], "data":item})
          });
        }
    ...
    handleNetworkSelect(item){
      ...
      this.network.id = networkData.netWorkId
      this.network.name = networkData.netWorkName
      this.network.gateway = networkData.netWorkGateway
      this.network.subnet = networkData.netWorkSubnet
