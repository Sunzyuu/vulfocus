<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="vulName" width="150" :show-overflow-tooltip=true label="漏洞名称"></el-table-column>
      <el-table-column :show-overflow-tooltip=true prop="username" width="100" label="用户名"></el-table-column>
      <el-table-column prop="vulHost" width="200" :show-overflow-tooltip=true label="访问地址"></el-table-column>
      <el-table-column label="状态" width="85">
        <template slot-scope="{row}">
          <el-tag>{{row.containerStatus}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="vulDesc" :show-overflow-tooltip=true width="300" label="漏洞描述"></el-table-column>
      <el-table-column prop="combination_desc" label="操作" :show-overflow-tooltip=true>
        <template slot-scope="{row}">
          <el-button size="mini" type="primary" icon="el-icon-caret-left" v-if="row.containerStatus === 'stop'"
                     @click="startContainer(row)" >启动</el-button>
          <el-button size="mini" type="primary" icon="el-icon-loading" v-if="row.containerStatus === 'running'"
                     @click="stopContainer(row)" >停止</el-button>
          <el-button size="mini" type="danger" icon="el-icon-delete" v-if="row.containerStatus === 'running' || row.containerStatus === 'stop'"
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      stopContainer(row){
        containerStop(row.containerId).then(response => {
      ...
      startContainer(row){
        containerStart(row.containerId).then(response => {
          let taskId = response.data["data"]
      ...
      delContainer(row){
        containerDel(row.containerId).then(response => {
          let taskId = response.data["data"]
          ...
          ImgList(queryString, true, 1).then(response => {
            ...
            if(results !== null){
              results.forEach((item, index, arr) => {
                this.imageList.push({"value": item["imageName"], "id": item["imageId"]})
              });
            }
